...
ap.add_argument("-i", "--inp",type=str,default="test_directory_folder",
help="path to input video")
ap.add_argument("-o", "--out", type=str, default='protected_directory_folder',
help="Output folder")
...
def blurring(image, image_tensor,tensor_dict):#, height, width):
    ...
    for ix in range(x):
        if(class_idd[ix]==1 or class_idd[ix]==72 or class_idd[ix]==73 or class_idd[ix]==70 or class_idd[ix]==76 or class_idd[ix]==77 or class_idd[ix]==84):
            #select the class indices which is your interest
...
if __name__ == '__main__':
  with detection_graph.as_default():
    with tf.Session() as sess:
        ...
        image_tensor, tensor_dict = initialization(detection_graph, 720, 1280)   #enter the size of input frame
   ...
   #          height, width = image_np_expanded.shape[1], image_np_expanded.shape[2]
            ...
            while(ret):
                img_processed = blurring(frame, image_tensor, tensor_dict)
